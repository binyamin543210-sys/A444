...
function getCity() {
  return state.settings.city || "Tel Aviv,IL";
}

// --- Open-Meteo helpers (מזג אוויר בלי API KEY) ---

async function geocodeCity(name) {
  // Presets for cities – כולל בעברית
  const presets = {
    "tel aviv": { lat: 32.0853, lon: 34.7818, tz: "Asia/Jerusalem" },
    "tel aviv,il": { lat: 32.0853, lon: 34.7818, tz: "Asia/Jerusalem" },
    "תל אביב": { lat: 32.0853, lon: 34.7818, tz: "Asia/Jerusalem" },
    "תל אביב-יפו": { lat: 32.0853, lon: 34.7818, tz: "Asia/Jerusalem" },
    "jerusalem": { lat: 31.778, lon: 35.235, tz: "Asia/Jerusalem" },
    "jerusalem,il": { lat: 31.778, lon: 35.235, tz: "Asia/Jerusalem" },
    "ירושלים": { lat: 31.778, lon: 35.235, tz: "Asia/Jerusalem" }
  };
  ...
  if (key && presets[key]) {
    const p = presets[key];
    state.settings.cityLat = p.lat;
    state.settings.cityLon = p.lon;
    state.settings.cityTz = p.tz;
    try {
      ...
      console.warn("Failed saving preset city coords", e);
    }
    return;
  }

  // אחרת – ניסיון דרך Open-Meteo
  try {
    ...
    console.warn("geocodeCity failed, falling back to Tel Aviv:", e);
    const fb = presets["tel aviv"];
    if (fb) {
      state.settings.cityLat = fb.lat;
      state.settings.cityLon = fb.lon;
      state.settings.cityTz = fb.tz;
      try {
        const settingsRef = ref(db, "settings");
        update(settingsRef, {
          cityLat: state.settings.cityLat,
        ...
        console.warn("Failed saving fallback city coords", e2);
      }
      return;
    }
    throw e;
...
async function ensureShabbatForWeek(fridayDate) {
  ...
  if (!state.settings.cityLat || !state.settings.cityLon || !state.settings.cityTz) {
    try {
      ...
      console.error("Failed to init city coords for shabbat", e);
      return null;
    }
  }
...
function renderCalendar() {
  ...
  // תחילת שבוע ביום ראשון (0=ראשון, 1=שני...)
...
function postponeTask(task) {
  // פותח חלונית דחיה במקום להזיז לבד
  state.ui.postponeTask = task;
  openPostponeModal(task);
}

function openPostponeModal(task) {
  const modal = el("postponeModal");
  const form = el("postponeForm");
  if (!modal || !form) return;

  const baseDate =
    task.dateKey && task.dateKey !== "undated" ? parseDateKey(task.dateKey) : new Date();
  const dk = dateKeyFromDate(baseDate);

  form.reset();
  form.elements["date"].value = dk;
  form.elements["startTime"].value = task.startTime || "20:00";
...
function handlePostponeFormSubmit(ev) {
  ev.preventDefault();
  const modal = el("postponeModal");
  const form = ev.target;
  const task = state.ui.postponeTask;
  if (!task) {
    if (modal) modal.classList.add("hidden");
    return;
  }

  const newDateKey = form.elements["date"].value;
  const startTime = form.elements["startTime"].value || "20:00";
  ...
  const toRef = ref(db, `events/${newDateKey}/${task.id}`);
  ...
  set(toRef, newData);
  if (task.dateKey && task.dateKey !== newDateKey) {
    remove(fromRef);
  }

  state.ui.postponeTask = null;
  if (modal) modal.classList.add("hidden");
}
...
function openDayModal(date) {
  ...
  el("dayModalHeb").innerHTML = `<span class="day-letter">${dayName(date)}</span> ${formatHebrewDate(date)}`;
...
function renderDayEvents(dateKey) {
  ...
  list.forEach((ev) => {
    ...
    delBtn.onclick = (e) => {
      e.stopPropagation();
      if (!confirm("למחוק את האירוע/משימה?")) return;
...
async function fetchWeatherForDate(date, autoShowIfEmpty) {
  ...
  try {
    ...
    el("dayWeatherDesc").textContent = mapped.label;
...
function applyBackground(idx) {
  document.body.setAttribute("data-bg", String(idx));
  try {
    localStorage.setItem("bnappBg", String(idx));
  } catch (e) {}
}
...
function initBackgrounds() {
  let idx = 1;
  try {
    const saved = localStorage.getItem("bnappBg");
    if (saved) {
      idx = parseInt(saved, 10) || 1;
    }
  } catch (e) {}
  applyBackground(idx);
  const dots = qsa(".bg-dot");
  dots.forEach((btn) => {
    if (btn.dataset.bg === String(idx)) {
      btn.classList.add("active");
    }
    btn.addEventListener("click", () => {
      const val = btn.dataset.bg || "1";
      applyBackground(val);
      dots.forEach((b) => b.classList.toggle("active", b === btn));
    });
  });
}
...
function initApp() {
  ...
  if (userSel) {
    ...
    userSel.addEventListener("change", () => {
      state.currentUser = userSel.value;
      renderCalendar();
      renderTasks();
      updateStats();
    });
  ...
  el("btnAddTask").onclick = () => openEditModal({ dateKey: "undated" });
  el("btnCity").onclick = () => {
  ...
  const postponeForm = el("postponeForm");
  if (postponeForm) {
    postponeForm.addEventListener("submit", handlePostponeFormSubmit);
  }

  const userSelectEl = el("userSelect");
  if (userSelectEl) {
    userSelectEl.addEventListener("change", (e) => {
      state.currentUser = e.target.value || "binyamin";
      renderCalendar();
      renderTasks();
      updateStats();
    });
...
function handleGihariVoiceCommand(text) {
  ...
  // יצירת משימה פשוטה – לדוגמה: "תוסיף לי משימה לקנות חלב מחר בשעה חמש"
  if (text.includes("משימה") && !text.includes("תוסיף לי הופעה") && !text.includes("אירוע")) {
    const targetDate = parseCommandTargetDate(text);
    const hour = parseCommandHour(text) ?? 17;
    const dk = dateKeyFromDate(targetDate);
    let title = "משימה";

    const taskIdx = text.indexOf("משימה");
    if (taskIdx >= 0) {
      const after = text.slice(taskIdx + "משימה".length).trim();
      if (after) {
        title = after;
      }
    }
    ...
    appendGihariLog(
      `הוספתי משימה "<strong>${title}</strong>" ל־${dk} בשעה ${startH}:${startM}.`
    );
    return;
  }
