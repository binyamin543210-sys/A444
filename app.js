...
function getCity() {
  return state.settings.city || "תל אביב";
}
...
async function ensureShabbatForWeek(fridayDate) {
  ...
  if (!state.settings.cityLat || !state.settings.cityLon || !state.settings.cityTz) {
    try {
      ...
      console.error("Failed to ensure city coords for shabbat times", e);
      return null;
    }
  }
...
function showFreeTimeForToday() {
  ...
  if (!freeSlots.length) {
    appendGihariLog("היום הזה מפוצץ. לא מצאתי חלון פנוי רציני.");
    return;
  }
  let msg = "הנה חלונות זמן פנויים היום:\n";
  freeSlots.forEach(([start, end], idx) => {
...
function handleGihariVoiceCommand(text) {
  ...
  // בקשות "מה אני עושה עכשיו" / "מה לעשות עכשיו" -> המלצה מיידית
  if (
    text.includes("מה לעשות עכשיו") ||
    text.includes("מה אני עושה עכשיו") ||
    text.includes("מה כדאי לעשות עכשיו") ||
    text.includes("מה לעשות") && text.includes("עכשיו")
  ) {
    gihariSuggestNow();
    return;
  }

  // בקשות "מתי אני פנוי היום" / "תראה לי זמן פנוי"
  if (
    text.includes("מתי אני פנוי") ||
    text.includes("מתי יש לי זמן היום") ||
    text.includes("זמן פנוי היום") ||
    (text.includes("זמן פנוי") && text.includes("היום"))
  ) {
    showFreeTimeForToday();
    return;
  }

  // בקשות "שבץ משימות ללא תאריך" וכדומה
  if (
    (text.includes("שבץ") || text.includes("סדר") || text.includes("תארגן")) &&
    (text.includes("משימות ללא תאריך") || text.includes("משימות") || text.includes("משימה"))
  ) {
    gihariPlaceUndatedTasks();
    return;
  }
